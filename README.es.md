<p align="right">
  <a href="./README.md">🇧🇷 Português</a> ·
  <a href="./README.en.md">🇺🇸 English</a> ·
  <a href="./README.es.md"><strong>🇪🇸 Español</strong></a>
</p>

---

<div>
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&width=435&lines=Bienvenido!;Me+llamo+Jo%C3%A3o+Vitor;Soy+estudiante+de+An%C3%A1lisis+de+Sistemas;Disfruta+mis+proyectos!" alt="Typing SVG" />
  <img 
  src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExOG9oZ3d2OWxsZno4aGF6ZjRtMmFhOXNtY2Vzem9haXZwZHJiYzFzYSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/jBOOXxSJfG8kqMxT11/giphy.gif"
  width="230px"
  align="right"
  style="border-radius: 50%; margin-left: 40px; box-shadow: 0 0 10px rgba(0,0,0,0.25);"
/>

</div>

<br><br>

<h3>👋 ¡Hola! Soy João Vitor Sgobin</h3>

Soy un desarrollador apasionado por la tecnología desde niño — ese tipo de persona que pasaba horas jugando videojuegos, explorando la computadora y preguntándose cómo funcionaba todo detrás de las pantallas.  
Con el tiempo, la curiosidad se convirtió en motivación, y la motivación en carrera.

<hr>

<h3>🚀 Mi trayectoria</h3>

Nací el 16 de marzo de 2006 y, desde temprano, la programación llamó mi atención. Siempre quise entender cómo se creaban los juegos que jugaba, cómo funcionaban los sistemas y cómo era posible transformar ideas en algo real.

En 2020, decidí seguir este camino oficialmente: ingresé a la ETEC en el curso de Desarrollo de Sistemas, donde también tuve la oportunidad de participar en proyectos con mentoría de IBM. Esa experiencia fortaleció aún más mi conexión con el mundo de la tecnología.

Actualmente, estoy finalizando mi formación en Análisis y Desarrollo de Sistemas en la FATEC Americana, ampliando mis conocimientos en desarrollo web, mobile, backend, bases de datos, cloud e IoT.

<hr>

<h3>💻 Lo que más me gusta hacer</h3>

<ul>
  <li>Desarrollo Web (Frontend & Backend)</li>
  <li>Proyectos con Firebase, Java Spring, Node.js, Python</li>
  <li>Aplicaciones conectadas y sistemas basados en IoT</li>
  <li>Crear soluciones completas: idea ➝ código ➝ despliegue</li>
  <li>Transformar problemas reales en herramientas útiles</li>
</ul>

<hr>

<h2 align="left">🚀 Herramientas y habilidades:</h2>

<div>
  <img height="50px" src="https://assets.univesp.br/novotec/codepen/unidade4/html5.png"/>
  <img height="50px" src="https://logospng.org/download/css-3/logo-css-3-2048.png"/>
  <img height="50px" src="https://logospng.org/download/javascript/logo-javascript-icon-1024.png"/>
  <img height="50px" src="https://www.cursou.com.br/wp-content/uploads/2017/10/Curso-de-PHP-PSRs.png"/>
  <img height="50px" src="https://www.freepnglogos.com/uploads/logo-mysql-png/logo-mysql-mysql-logo-png-images-are-download-crazypng-21.png"/>
  <img height="50px" src="https://runestone.academy/ns/books/published/mobilecsp/_images/python-logo.png"/>
  <img height="50px" src="https://upload.wikimedia.org/wikipedia/pt/3/30/Java_programming_language_logo.svg"/>
</div>

<hr>

<p>🤝 Si deseas trabajar conmigo o conversar, no dudes en enviarme un mensaje:</p>

<p align="left">
 <a href="https://github.com/ssgobin" target="_blank">
    <img src="https://img.shields.io/badge/-Github-242424?style=for-the-badge&logo=Github&logoColor=white">
 </a>

 <a href="https://www.linkedin.com/in/joao-vitor-sgobin-4a4556211/" target="_blank">
    <img src="https://img.shields.io/badge/-Linkedin-0e76a8?style=for-the-badge&logo=Linkedin&logoColor=white" />
 </a>

 <a href="mailto:ssgobin.dev@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/-Gmail-cc3838?style=for-the-badge&logo=Gmail&logoColor=white" />
 </a>
</p>

[![Spotify](https://novatorem-oc6h0x6ww-ssgobin.vercel.app/api/spotify)](https://open.spotify.com/user/ssgobin)
